...
1. Loading the provided `.pcap`, I saw that it contained 33,279 packets. The scenario mentions that the threat actor uploaded a web shell, so I searched for `http.request.method == "POST" && http.content_type contains "multipart"`. This returned two packets, both from `23[.]158[.]56[.]196` to the server at `172[.]31[.]25[.]119`. The uploaded file was named `NSt8bHTg.zip`.

2. With the threat actor and compromised web server IP addresses known, I filtered for HTTP responses from that server to the threat actor with `http.response && ip.src == 172[.]31[.]25[.]119 && ip.dst == 23[.]158[.]56[.]196`. An XML response provided server build information, including version `2023.11.3`.

3. The build information confirmed that the web server was running JetBrains TeamCity. Searching for vulnerabilities affecting this build yielded [`CVE-2024-27198`](https://nvd.nist.gov/vuln/detail/CVE-2024-27198), an authentication bypass affecting TeamCity On-Premises `2023.11.3` and earlier.

4. Searching for `ip.src == 23[.]158[.]56[.]196 && ip.dst == 172[.]31[.]25[.]119`, I saw the threat actor make a POST request to `/hax?jsp=/app/rest/users;.jsp` containing credentials for a new user: `c91oyemw:CL5v********`. This request shows the authentication bypass in action. `/app/rest/users` normally requires an authenticated administrator, but the trailing `;.jsp` causes TeamCity's servlet mapping to treat the request as a `.jsp` file and skip the authentication filter, while the `?jsp=` parameter still routes it to the privileged REST endpoint. The attacker created an administrator account without first authenticating.

5. I followed the HTTP stream where the threat actor uploaded the malicious `.zip` and traced their actions through the requests. The first command was `ls` at `2024-06-30 08:03`. Shortly thereafter, they ran `whoami`, and the server returned `root`, confirming that the web shell was executing with full privileges rather than as a service account.
...
7. The threat actor's modification of data at rest maps to [`T1565.001` Data Manipulation: Stored Data Manipulation](https://attack.mitre.org/techniques/T1565/001/).
...
- Timestamp unavailable: At approximately 2024-06-30 08:14 UTC, the threat actor attempted a container escape via `docker run --rm -it -v /:/host ubuntu chroot /host`. The capture ends here.
...
On June 30, 2024, an attacker at `23[.]158[.]56[.]196` compromised a public-facing JetBrains TeamCity server at `172[.]31[.]25[.]119` by exploiting `CVE-2024-27198`. The server disclosed its version, `2023.11.3`, in an XML build-information response, and that version falls within the affected range. The vulnerability lets anyone with HTTP access to the server reach authenticated REST endpoints without credentials by appending `;.jsp` to the request path, which causes the authentication filter to be skipped while the request still resolves to the privileged endpoint. The capture shows it used that way: a single unauthenticated POST request to `/hax?jsp=/app/rest/users;.jsp` created the administrator account `c91oyemw` less than a second after the version probe.
...
The confirmed impact is data manipulation. The attacker enumerated the filesystem, located `/tmp/Creds.txt`, and rewrote its credentials at 08:13:56, which maps to `T1565.001`. That root shell was inside the TeamCity container rather than on the host, and the final requests in the capture show an attempt to cross that boundary: `docker run --rm -it -v /:/host ubuntu chroot /host` bind-mounts the host's root filesystem into a new container and chroots into it. The capture ends at that point, so this evidence does not establish whether the escape succeeded or whether the attacker reached the underlying host. The host must be treated as compromised until disk-level artifacts establish otherwise.

The PCAP is the only artifact provided, and it does not record how the intrusion was noticed. Nothing in the traffic indicates that a preventive control interrupted the attacker at any stage; the sequence from first probe to credential tampering completed in about eleven minutes without interference.
