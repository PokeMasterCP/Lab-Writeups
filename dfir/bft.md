...
| --- | --- | --- | --- |
| BFT | Hack The Box | Very Easy | MFT forensics |
...
1. With the provided KAPE triage package, I parsed the Windows Master File Table (MFT) so I could review it in Timeline Explorer:

    ```powershell
    & 'C:\Tools\Zimmerman Tools\net9\MFTECmd.exe' -f ".\C\`$MFT" --csv 'C:\Users\Christian\Desktop\BFT\' --csvf MFT_ANALYSIS.csv
    ```

    I opened the parsed `MFT_ANALYSIS.csv` file with Timeline Explorer. The first question asked about a ZIP file received on February 13. I filtered the **Extension** column for `.zip`, which yielded one result: `Stage-20240213T093324Z-001.zip`, created at `2024-02-13 16:34:40`.
...
`invoice.bat` is small enough to be resident in its MFT record, so the script body was recoverable directly from `$MFT` at offset `0x16E3000` without the file itself. Its contents show the script reaching out to `43[.]204[.]110[.]203` on TCP 6666, making it a stager for an attacker-controlled command-and-control (C2) channel.
